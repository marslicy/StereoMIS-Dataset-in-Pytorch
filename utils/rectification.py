...
def get_rect_maps(
    ...
    if mode == "conventional":
        if triangular_intrinsics:
            lcam_mat = np.array(
                [
                    [lcam_mat[0, 0], 0, lcam_mat[0, 2]],
                    [0, lcam_mat[1, 1], lcam_mat[1, 2]],
                    [0, 0, 1],
                ],
                dtype=np.float64,
            )
            rcam_mat = np.array(
                [
                    [rcam_mat[0, 0], 0, rcam_mat[0, 2]],
                    [0, rcam_mat[1, 1], rcam_mat[1, 2]],
                    [0, 0, 1],
                ],
                dtype=np.float64,
            )
        ...
        r1, r2, p1, p2, q, valid_pix_roi1, valid_pix_roi2 = cv2.stereoRectify(
            cameraMatrix1=lcam_mat.astype("float64"),
            distCoeffs1=ldist_coeffs.astype("float64"),
            cameraMatrix2=rcam_mat.astype("float64"),
            distCoeffs2=rdist_coeffs.astype("float64"),
            imageSize=tuple(img_size),
            R=rmat.astype("float64"),
            T=tvec.T.astype("float64"),
            alpha=0,
        )
        lmap1, lmap2 = cv2.initUndistortRectifyMap(
            cameraMatrix=lcam_mat,
            distCoeffs=ldist_coeffs,
            R=r1,
            newCameraMatrix=p1,
            size=tuple(img_size),
            m1type=cv2.CV_32FC1,
...
def rectify_pair(limg, rimg, maps, method="nearest"):
    ...
    limg_rect = cv2.remap(
        np.copy(limg), maps["lmap1"], maps["lmap2"], interpolation=cv_interpol
    )
    rimg_rect = cv2.remap(
        np.copy(rimg), maps["rmap1"], maps["rmap2"], interpolation=cv_interpol
    )
...
class StereoRectifier(object):
    def __init__(self, calib_file, img_size_new=None, mode="conventional"):
        ...
        cal["img_size"] = (int(cal["img_size"][0]), int(cal["img_size"][1]))
    ...
    def get_rectified_calib(self):
        ...
        if self.mode == "conventional":
            calib_rectifed["extrinsics"][:3, 3] = np.array(
                [self.r_intr[0, 3] / self.r_intr[0, 0], 0.0, 0.0]
            )  # Tx*f, see cv2 website
        else:
            calib_rectifed["extrinsics"][:3, 3] = self.cal["T"]
        calib_rectifed["bf"] = (
            np.sqrt(np.sum(calib_rectifed["extrinsics"][:3, 3] ** 2))
            * self.l_intr[0, 0]
        )
    ...
    def _load_calib_ini(fname):
        ...
        config.read(fname)
        img_size = (
            float(config["StereoLeft"]["res_x"]),
            float(config["StereoLeft"]["res_y"]),
        )
        ...
        ld = np.array(
            [
                ...
                float(config["StereoLeft"]["kc_5"]),
                float(config["StereoLeft"]["kc_6"]),
                float(config["StereoLeft"]["kc_7"]),
            ]
        )
        rd = np.array(
            [
                ...
                float(config["StereoRight"]["kc_5"]),
                float(config["StereoRight"]["kc_6"]),
                float(config["StereoRight"]["kc_7"]),
            ]
        )
    ...
    def _load_calib_yaml(fname):
        ...
        img_size = (
            int(fs.getNode("Camera.width").real()),
            int(fs.getNode("Camera.height").real()),
        )
